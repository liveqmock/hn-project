...
public class HotelRoomController extends GenericController{
	...
	public String list(HotelRoom hotelRoom, HttpServletRequest request, Model model) {
		model.addAttribute("hotelRoom", hotelRoom);
		int pageSize = getParam(request, "ec_rd", 13);
		int pageNo = getParam(request, "ec_p", 1);
		/*获取总记录数*/
		int totalRows = (int)hotelRoomService.queryCount(hotelRoom);
		...
		Page<HotelRoom> page = hotelRoomService.queryList(hotelRoom, pageable);
		model.addAttribute("hotelRoomList", page.getContent());
	...
	public String listPage(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<HotelRoom> hotelRoomListPage = hotelRoomService.queryHotelRoomListPage(searchParams, pageNumber, pageSize, sortType);
		model.addAttribute("hotelRoomListPage", hotelRoomListPage);
	...
	public String toAdd(Model model) {
		...
		return "hotelRoom/hotelRoomAdd";
	}

	@RequestMapping(value = "add", method = RequestMethod.POST)
	public String add(@Valid HotelRoom hotelRoom, RedirectAttributes redirectAttributes) {
	...
	public String toUpdate(@PathVariable("id") Long id, Model model) {
		HotelRoom hotelRoom=hotelRoomService.getEntity(id);
		model.addAttribute("hotelRoom", hotelRoom);
		model.addAttribute("action", "update");
		return "hotelRoom/hotelRoomUpdate";
	}

	@RequestMapping(value = "update", method = RequestMethod.POST)
	public String update(@Valid @ModelAttribute("hotelRoom") HotelRoom hotelRoom, RedirectAttributes redirectAttributes) {
		hotelRoomService.save(hotelRoom);
		redirectAttributes.addFlashAttribute("message", "更新任务成功");
		return "redirect:/hotelRoom/list?id=2";
	}
